...
void Box2dRenderer::DrawPolygon(const b2Vec2* vertices, int32 vertexCount, const b2Color& color)
{
    ...
    {
        pixelboost::Game::Instance()->GetPrimitiveRenderer()->AttachLine(_Layer, Vec2(vertices[i].x, vertices[i].y), Vec2(vertices[i+1].x, vertices[i+1].y), Vec4(color.r, color.g, color.b, 1.f));
    }
    
    if (vertexCount)
    {
        pixelboost::Game::Instance()->GetPrimitiveRenderer()->AttachLine(_Layer, Vec2(vertices[vertexCount-1].x, vertices[vertexCount-1].y), Vec2(vertices[0].x, vertices[0].y), Vec4(color.r, color.g, color.b, 1.f));
    }
}
...
{
    pixelboost::Game::Instance()->GetPrimitiveRenderer()->AttachEllipse(_Layer, Vec2(center.x, center.y), Vec2(radius, radius), Vec3(color.r, color.g, color.b));
}
...
{
    pixelboost::Game::Instance()->GetPrimitiveRenderer()->AttachLine(_Layer, Vec2(p1.x, p1.y), Vec2(p2.x, p2.y), Vec4(color.r, color.g, color.b, 1.f));
}
