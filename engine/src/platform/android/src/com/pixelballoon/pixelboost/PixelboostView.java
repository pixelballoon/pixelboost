...
class PixelboostView extends com.pixelballoon.pixelboost.GLSurfaceView {
    ...
    public boolean onTouchEvent(MotionEvent evt)
    {
        ...
        int x = (int)evt.getX(evt.getActionIndex());
        int y = (int)evt.getY(evt.getActionIndex());

        switch (evt.getActionMasked())
        {
            case MotionEvent.ACTION_POINTER_DOWN:
            case MotionEvent.ACTION_DOWN:
                PixelboostLib.onPointerDown(pointerId, x, y);
                break;
            case MotionEvent.ACTION_POINTER_UP:
            case MotionEvent.ACTION_UP:
            case MotionEvent.ACTION_CANCEL:
                PixelboostLib.onPointerUp(pointerId, x, y);
                break;
            case MotionEvent.ACTION_MOVE:
                int pointerCount = evt.getPointerCount();
                for (int i = 0; i < pointerCount; i++)
                {
                    pointerId = evt.getPointerId(i);
                    x = (int)evt.getX(i);
                    y = (int)evt.getY(i);
                    PixelboostLib.onPointerMove(pointerId, x, y);
                }
    ...
    private static class Renderer implements GLSurfaceView.Renderer
    {
        ...
        {
            PixelboostLib.init(width, height);
        }

        public void onSurfaceCreated(GL10 gl, EGLConfig config)
        {
            PixelboostLib.onSurfaceCreated();
        }
    }
}
