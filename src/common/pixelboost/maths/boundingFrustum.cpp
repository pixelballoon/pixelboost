...
void BoundingFrustum::Set(const glm::mat4x4& viewProjectionMatrix)
{
    ...
    front.x = viewProjectionMatrix[0][2];
    front.y = viewProjectionMatrix[1][2];
    front.z = viewProjectionMatrix[2][2];
    front.w = viewProjectionMatrix[3][2];
...
bool BoundingFrustum::Contains(glm::vec3 point)
{
    return false;
}

bool BoundingFrustum::Intersects(const BoundingSphere& sphere)
{
    for (int i=0; i<6; i++)
    {
        if (glm::dot(glm::vec4(sphere.GetCenter(), 1), _Planes[i]) + sphere.GetSize() < 0)
            return false;
    }
...
void BoundingFrustum::Normalise()
{
    for (int i=0; i<6; i++)
    {
        _Planes[i] = glm::vec4(glm::normalize(glm::vec3(_Planes[i].x, _Planes[i].y, _Planes[i].z)), _Planes[i].w);
    }
}
